...
export default function AgenteForm() {
    ...
    if (registrosSinConfirmar.length > 0) {
      try {
        ...
        if (data.results && Array.isArray(data.results)) {
          setRegistros([]) // Resetear los registros después de confirmar
        } else {
          console.error('Formato de respuesta inesperado:', data)
        }
      } catch (error) {
        console.error('Error al confirmar registros:', error)
      }
    }
  }
  ...
  const handleLimpiar = () => {
    const form = document.getElementById('agenteForm') as HTMLFormElement
    if (form) form.reset()
  }

  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold">Formulario para Agente</h2>
      <form id="agenteForm" onSubmit={handleSubmit} className="space-y-6">
        <BaseForm />
